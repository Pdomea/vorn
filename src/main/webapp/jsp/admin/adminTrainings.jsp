...
<div class="card admin-trainings-form-card">
    <h2><%= adminTrainingsBean.getFormHeadline() %></h2>
    <form class="admin-form" method="post" action="<%= request.getContextPath() %>/admin/training/save">
        <input type="hidden" name="id" value="<%= adminTrainingsBean.getFormId() %>">

        <label for="title">Titel</label>
        <input id="title" name="title" type="text" value="<%= adminTrainingsBean.getFormTitle() %>" required>
        ...
        <div class="form-row-actions">
            <button class="btn btn-primary" type="submit"><%= adminTrainingsBean.getSubmitLabel() %></button>
            <a class="btn btn-ghost" href="<%= request.getContextPath() %>/admin/trainings">Formular leeren</a>
        </div>
    </form>
</div>
...
<div class="card admin-trainings-table-card">
    ...
    <form class="admin-filter-bar" method="get" action="<%= request.getContextPath() %>/admin/trainings">
        ...
        <div class="filter-group">
            ...
            <select id="trainingStatus" name="status">
                ...
                <option value="PUBLISHED" <%= "PUBLISHED".equals(statusFilter) ? "selected" : "" %>>PUBLISHED</option>
                <option value="HIDDEN" <%= "HIDDEN".equals(statusFilter) ? "selected" : "" %>>HIDDEN</option>
            </select>
        </div>
    ...
    <div class="table-sort-row">
        <a class="btn btn-ghost <%= "id".equals(sortBy) && "asc".equals(sortDir) ? "is-active" : "" %>"
           href="<%= request.getContextPath() %>/admin/trainings?sortBy=id&sortDir=asc<%= statusQuerySuffix %>">ID aufsteigend</a>
        <a class="btn btn-ghost <%= "id".equals(sortBy) && "desc".equals(sortDir) ? "is-active" : "" %>"
           href="<%= request.getContextPath() %>/admin/trainings?sortBy=id&sortDir=desc<%= statusQuerySuffix %>">ID absteigend</a>
        <a class="btn btn-ghost <%= "title".equals(sortBy) && "asc".equals(sortDir) ? "is-active" : "" %>"
           href="<%= request.getContextPath() %>/admin/trainings?sortBy=title&sortDir=asc<%= statusQuerySuffix %>">Titel A-Z</a>
        <a class="btn btn-ghost <%= "title".equals(sortBy) && "desc".equals(sortDir) ? "is-active" : "" %>"
           href="<%= request.getContextPath() %>/admin/trainings?sortBy=title&sortDir=desc<%= statusQuerySuffix %>">Titel Z-A</a>
    </div>
        ...
        <tr>
            ...
            <td>
                <%
                    ...
                    } else if ("HIDDEN".equals(training.getStatus())) {
                        statusClass = "status-in-progress";
                    }
                %>
                <span class="status-badge <%= statusClass %>"><%= training.getStatus() %></span>
            </td>
            <td><%= training.getDescription() == null ? "" : training.getDescription() %></td>
            <td class="table-actions">
                <a class="btn btn-ghost"
                   href="<%= request.getContextPath() %>/admin/trainings?editId=<%= training.getId() %><%= sortQuerySuffix %><%= statusQuerySuffix %>">Bearbeiten</a>
                <a class="btn btn-secondary" href="<%= request.getContextPath() %>/admin/training/exercises?trainingId=<%= training.getId() %>">Übungen zuordnen</a>

                <% if (!"PUBLISHED".equals(training.getStatus())) { %>
                <form class="inline-form" method="post" action="<%= request.getContextPath() %>/admin/training/publish">
                ...
                <form class="inline-form" method="post" action="<%= request.getContextPath() %>/admin/training/hide">
                    <input type="hidden" name="id" value="<%= training.getId() %>">
