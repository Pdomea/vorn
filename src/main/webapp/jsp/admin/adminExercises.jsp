<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    <%@ page import="java.util.List" %>
        <%@ page import="app.model.Exercise" %>
                        ...
                        <% List<Exercise> exercises = adminExercisesBean.getExercises();
                            List<MuscleGroup> muscleGroups = adminExercisesBean.getMuscleGroups();
                                ...
                                <% if (adminExercisesBean.getInfo() !=null && !adminExercisesBean.getInfo().isBlank()) {
                                    ...
                                    <% } %>
                                        <% if (adminExercisesBean.getError() !=null &&
                                            !adminExercisesBean.getError().isBlank()) { %>
                                            <div class="error">${adminExercisesBean.error}</div>
                                            <% } %>

                                                <div class="card admin-exercises-form-card">
                                                    <h2>
                                                        <%= adminExercisesBean.getFormHeadline() %>
                                                    </h2>
                                                    <form class="admin-form" method="post"
                                                        action="<%= request.getContextPath() %>/admin/exercise/save">
                                                        <input type="hidden" name="id"
                                                            value="<%= adminExercisesBean.getFormId() %>">

                                                        <label for="name">Name</label>
                                                        <input id="name" name="name" type="text"
                                                            value="<%= adminExercisesBean.getFormName() %>" required>
                                                        ...
                                                        <div class="muscle-group-grid">
                                                            <% if (!adminExercisesBean.hasMuscleGroups()) { %>
                                                                <p class="muted">Keine Muskelgruppen vorhanden. Bitte
                                                                    zuerst Migration 010 ausführen.</p>
                                                                <% } else { %>
                                                                    <% for (MuscleGroup muscleGroup : muscleGroups) {
                                                                        boolean
                                                                        checked=adminExercisesBean.isSelectedMuscleGroup(muscleGroup.getId());
                                                                        %>
                                                                        <label class="muscle-group-chip">
                                                                            <input type="checkbox" name="muscleGroupIds"
                                                        ...
                                                        <div class="form-row-actions">
                                                            <button class="btn btn-primary" type="submit">
                                                                <%= adminExercisesBean.getSubmitLabel() %>
                                                            </button>
                                                            <a class="btn btn-ghost"
                                                                href="<%= request.getContextPath() %>/admin/exercises">Formular
                                                                leeren</a>
                                                        </div>
                                                    </form>
                                                </div>
                                                ...
                                                <div class="card admin-exercises-table-card">
                                                    ...
                                                    <form class="admin-filter-bar" method="get"
                                                        action="<%= request.getContextPath() %>/admin/exercises">
                                                        <input type="hidden" name="sortBy" value="<%= sortBy %>">
                                                        <input type="hidden" name="sortDir" value="<%= sortDir %>">
                                                        <div class="filter-group">
                                                            <label for="exerciseStatus">Status</label>
                                                            <select id="exerciseStatus" name="status">
                                                                <option value="ALL" <%="ALL" .equals(statusFilter)
                                                                    ? "selected" : "" %>>Alle Status</option>
                                                                <option value="ACTIVE" <%="ACTIVE" .equals(statusFilter)
                                                                    ? "selected" : "" %>>ACTIVE</option>
                                                                <option value="ARCHIVED" <%="ARCHIVED"
                                                                    .equals(statusFilter) ? "selected" : "" %>>ARCHIVED
                                                                </option>
                                                            </select>
                                                        </div>
                                                    ...
                                                    <div class="table-sort-row">
                                                        <a class="btn btn-ghost <%= " name".equals(sortBy) && "asc"
                                                            .equals(sortDir) ? "is-active" : "" %>"
                                                            href="<%= request.getContextPath() %>
                                                                /admin/exercises?sortBy=name&sortDir=asc<%=
                                                                    statusQuerySuffix %>">Name A-Z</a>
                                                        <a class="btn btn-ghost <%= " name".equals(sortBy) && "desc"
                                                            .equals(sortDir) ? "is-active" : "" %>"
                                                            href="<%= request.getContextPath() %>
                                                                /admin/exercises?sortBy=name&sortDir=desc<%=
                                                                    statusQuerySuffix %>">Name Z-A</a>
                                                        <a class="btn btn-ghost <%= " muscle".equals(sortBy) && "asc"
                                                            .equals(sortDir) ? "is-active" : "" %>"
                                                            href="<%= request.getContextPath() %>
                                                                /admin/exercises?sortBy=muscle&sortDir=asc<%=
                                                                    statusQuerySuffix %>">Muskelgruppe A-Z</a>
                                                        <a class="btn btn-ghost <%= " muscle".equals(sortBy) && "desc"
                                                            .equals(sortDir) ? "is-active" : "" %>"
                                                            href="<%= request.getContextPath() %>
                                                                /admin/exercises?sortBy=muscle&sortDir=desc<%=
                                                                    statusQuerySuffix %>">Muskelgruppe Z-A</a>
                                                    </div>
            ...
            <td>
                <%
                    String statusClass = " status-open"; if ("ACTIVE".equals(exercise.getStatus())) {
                                                                                            statusClass="status-done" ;
                                                                                            } else if
                                                                                            ("ARCHIVED".equals(exercise.getStatus()))
                                                                                            {
                                                                            ...
                                                                            <td class="table-actions">
                                                                                ...
                                                                                <% if
                                                                                    ...
                                                                                    <form class="inline-form"
                                                                                        method="post"
                                                                                        action="<%= request.getContextPath() %>/admin/exercise/activate">
                                                                                        <input type="hidden" name="id"
                                                                                            value="<%= exercise.getId() %>">
                                                                                        <button class="btn btn-primary"
                                                                                    ...
                                                                                    <% } else { %>
                                                                                        <form class="inline-form"
                                                                                            method="post"
                                                                                            action="<%= request.getContextPath() %>/admin/exercise/archive">
                                                                                            <input type="hidden"
                                                                                                name="id"
                                                                                                value="<%= exercise.getId() %>">
                                                                                            <button
