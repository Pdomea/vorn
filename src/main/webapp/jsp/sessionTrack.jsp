...
<jsp:useBean id="sessionTrackBean" class="app.beans.SessionTrackBean" scope="request" />
<%
    WorkoutSession sessionData = sessionTrackBean.getSessionData();
    List<SessionExercise> items = sessionTrackBean.getItems();
    Map<Long, List<WorkoutLog>> logsByExercise = sessionTrackBean.getLogsByExercise();
    Map<Long, BigDecimal> lastScoreByExerciseId = sessionTrackBean.getLastScoreByExerciseId();
    Map<Long, List<Exercise>> swapCandidatesBySessionExerciseId = sessionTrackBean.getSwapCandidatesBySessionExerciseId();
    boolean activeSession = sessionTrackBean.isActiveSession();
    Long startedAtEpochMillis = sessionTrackBean.getStartedAtEpochMillis();
%>
...
<div class="page-header">
    <div>
        ...
        <% if (sessionData != null) { %>
        <p class="muted"><%= sessionData.getTrainingTitle() %></p>
        <% } %>
    </div>
    <div class="top-actions">
        <a class="btn btn-ghost" href="<%= request.getContextPath() %>/home">Zurück zu Home</a>
        <a class="btn btn-ghost" href="<%= request.getContextPath() %>/plan/details">Wochenplan</a>
    </div>
</div>

<% if (sessionTrackBean.getInfo() != null && !sessionTrackBean.getInfo().isBlank()) { %>
...
<div class="meta track-session-meta">
    <p><strong>Gestartet:</strong> <%= sessionData.getStartedAt() %></p>
    <% if (activeSession) { %>
    <p><strong>Dauer:</strong> <span id="sessionDuration">00:00:00</span></p>
    <% } %>
</div>
    ...
    <% } else { %>
        <% if (activeSession) { %>
        <form id="swapSubmitForm" method="post" action="<%= request.getContextPath() %>/session/track/swap" hidden>
            <input type="hidden" name="sessionId" value="<%= sessionData.getId() %>">
            <input type="hidden" name="sessionExerciseId" id="swapSessionExerciseId" value="">
            <input type="hidden" name="replacementExerciseId" id="swapReplacementExerciseId" value="">
        </form>
        <form method="post" action="<%= request.getContextPath() %>/session/finish">
            <input type="hidden" name="sessionId" value="<%= sessionData.getId() %>">
        <% } %>

        <% for (SessionExercise item : items) { 
            List<WorkoutLog> itemLogs = logsByExercise == null ? null : logsByExercise.get(item.getId());
            boolean hasSavedLogs = itemLogs != null && !itemLogs.isEmpty();
            java.util.Map<Integer, WorkoutLog> existingBySet = new java.util.HashMap<>();
            BigDecimal itemCurrentScore = BigDecimal.ZERO;
            if (itemLogs != null) {
                for (WorkoutLog log : itemLogs) {
                    existingBySet.put(log.getSetNo(), log);
                    if (log.getWeight() != null) {
                        itemCurrentScore = itemCurrentScore.add(log.getWeight().multiply(BigDecimal.valueOf(log.getReps())));
                    }
                }
            }
            BigDecimal lastScore = lastScoreByExerciseId == null ? null : lastScoreByExerciseId.get(item.getExerciseId());
            String lastScoreText = lastScore == null ? "-" : lastScore.stripTrailingZeros().toPlainString();
            String currentScoreText = itemCurrentScore.stripTrailingZeros().toPlainString();
            List<Exercise> swapCandidates = swapCandidatesBySessionExerciseId == null
                    ? null
                    : swapCandidatesBySessionExerciseId.get(item.getId());
        %>
        <div class="card track-exercise-card" id="exercise-<%= item.getId() %>">
            ...
            <% if (activeSession) { %>
            <div class="track-swap-row">
                <% if (hasSavedLogs) { %>
                <p class="muted track-swap-hint">Tausch gesperrt: Für diese Übung wurden bereits Sets gespeichert.</p>
                <% } else if (swapCandidates == null || swapCandidates.isEmpty()) { %>
                <p class="muted track-swap-hint">Keine passende Ersatzübung in derselben Muskelgruppe verfügbar.</p>
                <% } else { %>
                <div class="track-swap-form">
                    <select class="track-swap-select" data-session-exercise-id="<%= item.getId() %>" required>
                        <% for (Exercise candidate : swapCandidates) { %>
                        <option value="<%= candidate.getId() %>"><%= candidate.getName() %></option>
                        <% } %>
                    </select>
                    <button class="btn btn-ghost track-swap-submit"
                            type="button"
                            data-session-exercise-id="<%= item.getId() %>">Übung tauschen</button>
                </div>
                <% } %>
            </div>
            <% } %>
            <div class="score-strip">
                <div class="score-pill">Letzter Score: <strong><%= lastScoreText %></strong></div>
                <div class="score-pill">Aktueller Score:
                    <strong id="currentScore_<%= item.getId() %>"><%= currentScoreText %></strong>
                </div>
            </div>
            <table class="plan-table track-table">
                <thead>
                <tr>
                    <th>Satz</th>
                    <th>Reps</th>
                ...
                <tbody>
                <% for (int setNo = 1; setNo <= item.getPlannedSetsSnapshot(); setNo++) {
                    String rowKey = item.getId() + "_" + setNo;
                    WorkoutLog existing = existingBySet.get(setNo);
                    String repsValue = existing == null ? "" : String.valueOf(existing.getReps());
                    String weightValue = existing == null ? "" : String.valueOf(existing.getWeight());
                    String noteValue = existing == null || existing.getNote() == null ? "" : existing.getNote();
                %>
                <tr>
                    <td>
                        <%= setNo %>
                    ...
                    <td>
                        <% if (activeSession) { %>
                        <input name="reps_<%= rowKey %>" type="number" min="1" value="<%= repsValue %>"
                    ...
                    <td>
                        <% if (activeSession) { %>
                        <input name="weight_<%= rowKey %>" type="number" step="0.01" min="0" value="<%= weightValue %>"
                               data-score-group="<%= item.getId() %>"
                    ...
                    <td>
                        <% if (activeSession) { %>
                        <textarea name="note_<%= rowKey %>"><%= noteValue %></textarea>
                        <% } else { %>
                        <%= noteValue %>
                        <% } %>
                    </td>
        ...
        <% if (activeSession) { %>
        <p class="track-finish-actions">
            <button class="btn btn-primary" type="submit">Training beenden &amp; speichern</button>
        </p>
        </form>

        <form class="discard-form" method="post" action="<%= request.getContextPath() %>/session/discard"
              onsubmit="return confirm('Willst du dieses Training wirklich verwerfen? Nicht gespeicherte Eingaben gehen verloren.');">
            <input type="hidden" name="sessionId" value="<%= sessionData.getId() %>">
            <button class="btn btn-danger" type="submit">Training verwerfen</button>
        </form>
        <% } %>
    <% } %>
<% } %>
...
<script>
    (function () {
        ...
        document.querySelectorAll(".track-swap-submit").forEach(function (swapButton) {
            swapButton.addEventListener("click", function () {
                var sessionExerciseId = swapButton.getAttribute("data-session-exercise-id");
                var swapSelect = document.querySelector(".track-swap-select[data-session-exercise-id='" + sessionExerciseId + "']");
                if (!swapSubmitForm || !swapSessionExerciseId || !swapReplacementExerciseId || !swapSelect || !swapSelect.value) {
                    alert("Bitte zuerst eine Ersatzübung auswählen.");
                    return;
                }
                persistDraftInputs();
